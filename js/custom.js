...
function pricingTrigger() {
  $("[data-plan-active]").each(function () {
    ...
    if (plan == "monthly") {
      ...
      $("[data-pricing-trigger='" + id + "'].toggle").attr("data-target", "yearly").removeClass("active");
    } else if (plan == "yearly") {
      $("[data-pricing-trigger='" + id + "'][data-target='monthly']").removeClass("active");
      $("[data-pricing-trigger='" + id + "'][data-target='yearly']").addClass("active");
      $("[data-pricing-trigger='" + id + "'].toggle").attr("data-target", "monthly").addClass("active");
    }
  });
  ...
  $("[data-pricing-trigger]").on("click", function (e) {
    ...
    $("[data-plan-id='" + id + "'] .dynamic-value").each(function () {
      ...
      let billingText = target === "monthly" ? "/Per Month" : "/Per Year";

      if (target == "monthly") {
        $(this).text(monthPrice);
        $(this).siblings(".pricing-card-2__time").text(billingText);
        ...
        $("[data-pricing-trigger].toggle").attr("data-target", "yearly").removeClass("active");
      }
      if (target == "yearly") {
        $(this).text(yearPrice);
        $(this).siblings(".pricing-card-2__time").text(billingText);
        ...
        $("[data-pricing-trigger].toggle").attr("data-target", "monthly").addClass("active");
      }
    });
  });
}
